...
function openLumos(imageToOpen, galleryName) {
    ...
    lumosContainer.imagesLoaded(function () {
        lumosImage.fadeIn(animationSpeed);
    });
...
function getNextImage(direction) {
    ...
    if (direction == "next") {
        nextImage = $("[href='" + currentImage + "']").next(".lumos-link");
    }
    if (direction == "prev") {
        nextImage = $("[href='" + currentImage + "']").prev(".lumos-link");
    }

    if (nextImage.attr("data-lumos") != currentGalleryName) {
        return currentImage;
    }
